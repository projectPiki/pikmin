...
BOOL EXIDetach(s32 chan)
{
	...
	exi->state &= ~STATE_ATTACHED;
	__OSMaskInterrupts((OS_INTERRUPTMASK_EXI_0_EXT | OS_INTERRUPTMASK_EXI_0_TC | OS_INTERRUPTMASK_EXI_0_EXI) >> (3 * chan));
	OSRestoreInterrupts(enabled);
	return TRUE;
}
...
BOOL EXISelect(s32 chan, u32 dev, u32 freq)
{
	...
	if ((exi->state & STATE_SELECTED)
	    || chan != 2
	           && (dev == 0 && !(exi->state & STATE_ATTACHED) && !__EXIProbe(chan) || !(exi->state & STATE_LOCKED) || (exi->dev != dev))) {
		OSRestoreInterrupts(enabled);
		return FALSE;
	}
